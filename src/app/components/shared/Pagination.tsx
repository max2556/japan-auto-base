...
export default function Pagination({ page, pages, onClick }: PaginationProps) {
  const options = {
    left: 3,
    right: 3,
  };
  ...
  return (
    <div className="flex items-center justify-center gap-2 text-sm font-bold text-brand-dark mt-4 select-none">
      {/* Navigating Button Left */}
      <button
        onClick={() => onClick(page - 1 >= 0 ? page - 1 : 0)}
        className="w-8 h-8 shrink-0 grid place-content-center bg-brand-gray-100 rounded-5"
      >
        <svg
          width="24"
          height="24"
      ...
      {[...Array(pages)].map((_, index) => {
        if (index >= page - options.left && index <= page + options.right) {
          return (
            <button
              key={index}
              onClick={() => onClick(index)}
              className={
                "w-8 h-8 shrink-0 grid place-content-center rounded-5" +
                (index == page
                  ? " bg-brand-purple text-white"
                  : " bg-brand-gray-100 text-brand-dark/80")
              }
            >
              {index + 1}
            </button>
      ...
      <button
        onClick={() => onClick(page + 1 < pages - 1 ? page + 1 : pages - 1)}
        className="w-8 h-8 shrink-0 grid place-content-center bg-brand-gray-100 rounded-5"
      >
        <svg
          width="24"
